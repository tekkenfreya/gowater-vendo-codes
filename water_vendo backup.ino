...
const int targetPulses[] = {0, 450, 2250, 8550};
const unsigned long flowDebounceDelay = 10;
 ...
 void cashPay() {
  ...
  while (true) {
    ...
    if (currency >= prices[address[1]]) {
      ...
      // Pulse-based dispensing (accurate measurement only)
      noInterrupts();
      pulseCount = 0;
      interrupts();
      ...
      int neededPulses = targetPulses[address[1]];

      Serial.print("Starting dispense. Target: ");
      Serial.print(neededPulses);
      Serial.println(" pulses");

      while (pulseCount < neededPulses) {
        // Check for stalled flow
        unsigned long currentPulseCount = pulseCount;
        unsigned long lastPulse = lastFlowPulse;
        if (currentPulseCount > 10 && (millis() - lastPulse > 5000)) {
          Serial.println("ERROR: Flow stalled");
          break;
        }

        // Progress update every 500ms
        if (millis() - lastPulseCheck > 500) {
          Serial.print("Pulses: ");
          Serial.print(pulseCount);
          Serial.print(" / ");
          Serial.println(neededPulses);
  ...
  if (cancelled) {
    digitalWrite(slotActivator, LOW);  // Make sure slot closes if canceled
