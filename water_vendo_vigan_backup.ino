...
const float targetVolumes[] = {0.0, 1.0, 5.0, 20.6, 19.4};
const int litersLabel[] = {0, 1, 5, 20, 19};
// ====================================================
...
void updateSalesAddresses() {
  ...
  address[16 + NUM_PRODUCTS] = totalLiters;
  address[16 + NUM_PRODUCTS + 1] = totalRevenue;
  address[16 + NUM_PRODUCTS + 2] = totalTransactions;
}
...
void clearSales() {
  ...
  prefs.clear();
  prefs.end();
  for (int i = 1; i <= NUM_PRODUCTS; i++) saleCounts[i] = 0;
  totalLiters = totalRevenue = totalTransactions = 0;
  for (int i = 16; i <= 16 + NUM_PRODUCTS + 2; i++) address[i] = 0;
}
...
void sendSalesSMS(String number) {
  ...
  for (int i = 1; i <= NUM_PRODUCTS; i++) {
    msg += String(saleCounts[i]) + "x" + String(litersLabel[i]) + "L\n";
  }
  msg += "Liters: " + String(totalLiters) + "L\n" +
